<div class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Tenggat</h4>
            <p class="mt-1 text-sm">{{ $task->deadline?->format('d M Y') ?? 'Tidak ada' }}</p>
        </div>
        <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Progress Subtugas</h4>
    ...
    @if($deadline)
        @if($task->status === 'done')
            <div class="p-3 rounded-lg bg-success-50 dark:bg-success-900/20 border border-success-200 dark:border-success-800">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium text-success-700 dark:text-success-300">Tugas selesai</span>
                </div>
            </div>
        ...
        @else
            ...
            <div class="p-3 rounded-lg bg-gray-50 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-800">
                <div class="flex items-center gap-2">
                    <x-filament::icon icon="heroicon-o-calendar" class="w-5 h-5 text-gray-600 dark:text-gray-400" />
    ...
    @if($task->assignees->isNotEmpty())
        <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Ditugaskan ke</h4>
    ...
    @if($task->comments->isNotEmpty())
        <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Komentar ({{ $task->comments->count() }})</h4>
    ...
    @if($task->attachments->isNotEmpty())
        <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Lampiran ({{ $task->attachments->count() }})</h4>
