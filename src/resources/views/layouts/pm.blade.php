        ...
        <aside class="w-72 bg-white border-r border-gray-200 flex flex-col shrink-0">
            ...
            <div class="flex-1 overflow-y-auto px-4 py-3">
                ...
                <div class="space-y-1">
                    ...
                    <a href="{{ route('pm.dashboard') }}?task={{ $task->id }}" class="flex items-start gap-2 px-2 py-1.5 rounded-md hover:bg-gray-50 group">
                        <span class="mt-1 w-2 h-2 rounded-full shrink-0
                            {{ $task->status === 'done' ? 'bg-green-500' : ($task->status === 'on_progress' ? 'bg-blue-500' : ($task->status === 'revision' ? 'bg-orange-500' : ($task->status === 'pending_pm' ? 'bg-yellow-500' : ($task->status === 'pending_admin' ? 'bg-purple-500' : 'bg-gray-400')))) }}">
                        </span>
